...
'''!!!!!!!!!!!!!!!!!! для нормальных данных заменить на data = pd.read_csv("test.csv")!!!!!!!!!!!!!!!!!!!!!!!'''
data = pd.read_csv("test_old.csv")
...
def generate_data_bot(msg):
    ...
    global generate_data, x_gen, y_gen
    generate_data = data.loc[np.random.randint(100)].to_list()
    '''!!!!!!!!!!!!!!!!!! для нормальных данных заменить на x_gen = generate_data[:-1]!!!!!!!!!!!!!!!!!!!!!!!'''
    x_gen = [33.0, 2.0, 0.7061855670103093, 
             0.5664637765171905, 0.5417414771162637,
             0.5664637765171905, 0.5417414771162637,
               0.5644845391417889, 0.5644845391417889,
                 0.5034151268739858, 0.4767652810976935,
                   -0.2794154981989258, 0.9893582466233818,
                     -0.1455000338086135, -0.0088513092904038,
                       -0.9999608263946372, -0.2697967711570244,
                         0.9629172873477992, -0.7557495743542582,
                           -0.6548607339452852, 0.4288425808633576,
                             -0.0088513092904038, -0.9999608263946372,
                               -0.2697967711570244, 0.9629172873477992]
    print(x_gen)
